...
const mailer = (data, cb) => {
    let mailOptions = {
        from: '"Smart Receipt" <' + mailSettings.auth.user + ">",
        to: data.email,
...
const mailer2 = (data, cb) => {
    let mailOptions = {
        from: '"Smart Receipt" <' + mailSettings.auth.user + ">",
        to: data.email,
        subject: "Your Receipt just arrived!",
        html: "Hello "+data.customerName+"<h1>Greetings from Smart Receipt</h1><p>Your ReceiptId is : </p><b><h2>" + data.receiptId + "</h2></b>",
        attachments: [
            {
